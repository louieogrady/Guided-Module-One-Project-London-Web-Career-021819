    ...
    if response == "y"
      ...
      if titles.find {|titles| titles == release_answer} # checks if the user query matches the release title for that artist.
        ...
        if collection_response == "y"
          rel = Release.create(artist: auth_wrapper.get_artist(artist_search.results.first.id).name, title: release_information.title, released: release_information.year, genre: release_information.genre, format: release_information.format)
          @u.releases << rel
